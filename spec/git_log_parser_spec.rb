...
describe GithubMirror::GitLogParser do
  ...
  it "parses a commits-only log" do
    parsed = do_parse File.join(File.dirname(__FILE__), "commits-only-log.txt")

    expect(parsed.count).to eq(3)
    expect(parsed.first[:commit_lines].first).to eq("commit 4a8097c61e6d6a8d2d451e47cc689eafd4a8a598\n")
    expect(parsed.first[:file_lines]).to be_empty
    expect(parsed.first[:hunk_lines]).to be_empty
