...
class GithubMirror
  ...
  def run
    ...
    GithubMirror::CloneAll.new(filtered_repos, CLONE_BASE_DIR).run

    require 'github_mirror/symlink_updater'
    GithubMirror::SymlinkUpdater.new(filtered_repos, CLONE_BASE_DIR).run
  ...
  def repo_enum
    ...
    GithubMirror::CacheingThing.new(REPOSITORIES_FILE, Time.now - ttl) do
